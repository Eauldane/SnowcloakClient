namespace Snowcloak.Core.BbCode;
...
public static partial class BbCodeParser
{
    ...
    private const int MaxElements = 4_000;
    private const int MaxImages = 24;
    private const int MaxStyleDepth = 64;
    private const int MaxListDepth = 32;
    ...
    public static IReadOnlyList<BbCodeElement> Parse(string input, ISet<string> knownEmotes)
    {
        ...
        var truncatedInput = input.Length > MaxInputLength ? input[..MaxInputLength] : input;
        var sanitizedInput = truncatedInput.Replace("\r", string.Empty);
        ...
        while (currentIndex < span.Length)
        {
            ...
            {
                case "b":
                    PushStyle(styleStack, styleStack.Peek() with { Bold = true });
                    break;
                case "i":
                    PushStyle(styleStack, styleStack.Peek() with { Italic = true });
                    break;
                case "u":
                    PushStyle(styleStack, styleStack.Peek() with { Underline = true });
                    break;
                case "color":
                case "colour":
                    if (TryParseColor(tagArgument, out var color))
                    {
                        PushStyle(styleStack, styleStack.Peek() with { Color = color });
                    }
                    else
                    {
                        PushStyle(styleStack, styleStack.Peek());
                    }
                    break;
                case "size":
                    if (TryParseSize(tagArgument, out var sizePx, out var scale))
                    {
                        PushStyle(styleStack, styleStack.Peek() with { FontSize = sizePx, FontScale = scale });
                    }
                    else
                    {
                        PushStyle(styleStack, styleStack.Peek());
                    ...
                    break;
                case "center":
                    PushStyle(styleStack, styleStack.Peek() with { Alignment = BbCodeAlignment.Center });
                    break;
                case "right":
                    PushStyle(styleStack, styleStack.Peek() with { Alignment = BbCodeAlignment.Right });
                    break;
                case "align":
                    PushStyle(styleStack, styleStack.Peek() with { Alignment = ParseAlignment(tagArgument) });
                    break;
                case "url":
                    var url = string.IsNullOrWhiteSpace(tagArgument) ? null : tagArgument.Trim();
                    PushStyle(styleStack, styleStack.Peek() with { Url = url, UseTextAsUrl = string.IsNullOrWhiteSpace(url), Underline = true });
                    break;
                case "img":
                    ...
                    if (closingIndex >= 0)
                    {
                        ...
                        {
                            AddText(sanitizedInput[match.Index..(closingIndex + "[/img]".Length)], styleStack.Peek(), knownEmotes, elements);
                    ...
                    break;
                case "list":
                    PushList(listStack, ParseListContext(tagArgument));
                    PushStyle(styleStack, styleStack.Peek());
                    break;
                case "ul":
                    PushList(listStack, new ListContext(ListKind.Unordered, 1));
                    PushStyle(styleStack, styleStack.Peek());
                    break;
                case "ol":
                    PushList(listStack, ParseListContext(tagArgument, ListKind.Decimal));
                    PushStyle(styleStack, styleStack.Peek());
                    break;
                case "*":
    ...
    private static void PushStyle(Stack<BbCodeStyle> styleStack, BbCodeStyle style)
    {
        if (styleStack.Count >= MaxStyleDepth) return;
        styleStack.Push(style);
    }

    private static void PushList(Stack<ListContext> listStack, ListContext context)
    {
        if (listStack.Count >= MaxListDepth) return;
    ...
    [GeneratedRegex(@"\[(/?)(\w+|\*)(?:=([^\]]+))?\]", RegexOptions.Compiled, matchTimeoutMilliseconds: 2500)]
    private static partial Regex TagRegex();

    [GeneratedRegex(@":([a-zA-Z0-9_]+):", RegexOptions.Compiled, matchTimeoutMilliseconds: 2500)]
