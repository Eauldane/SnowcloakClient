...
public static class ProfileTagPolicy
{
    ...
    public static string GetTypeLabel(ProfileTagType type) => ProfileTagText.GetTypeLabel(type);

    public static IReadOnlyList<string> GetDefaultSuggestions(ProfileTagType type)
        => DefaultTagSuggestions.GetValueOrDefault(type) ?? [];
        ...
        int? maxResults = null)
    {
        ...
        var hasResultLimit = maxResults is > 0;
    ...
    public static HashSet<string> BuildKinkLookup(IEnumerable<UserProfileTagDto>? tags)
        => BuildTypeLookup(tags, ProfileTagType.Kink);

    public static string NormalizeForLookup(string? value) => ProfileTagText.NormalizeForLookup(value);
