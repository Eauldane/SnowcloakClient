...
internal sealed partial class CharaDataHubUi : WindowMediatorSubscriberBase
{
    ...
    protected override void DrawInternal()
    {
        ...
        using (var applicationTabItem = ImRaii.TabItem("Data Application", _openDataApplicationShared ? ImGuiTabItemFlags.SetSelected : ImGuiTabItemFlags.None))
        {
            if (applicationTabItem)
            {
                ...
                if (!_uiSharedService.IsInGpose)
                    ElezenImgui.AttachTooltip("Only available in GPose");
        ...
        if (_isHandlingSelf)
        {
            ElezenImgui.AttachTooltip("Cannot use creation tools while having Character Data applied to self.");
        }
    ...
    private void DrawFavorite(string id)
    {
        ...
        if (_configService.Current.FavoriteCodes.ContainsKey(id))
        {
            ...
            ElezenImgui.AttachTooltip($"Custom Description: {favorite?.CustomDescription ?? string.Empty}" + UiSharedService.TooltipSeparator
                ...
                foreach (var favorite in _filteredFavorites.OrderByDescending(k => k.Value.Favorite.LastDownloaded))
                {
                    UiSharedService.DrawGrouped(() =>
                    {
                        ...
                        if (metaInfoDownloaded)
                        {
                            ...
                            if (metaInfo != null)
                            {
                                ElezenImgui.AttachTooltip("Metainfo present" + UiSharedService.TooltipSeparator
                            ...
                            {
                                ElezenImgui.AttachTooltip("Metainfo could not be downloaded." + UiSharedService.TooltipSeparator
                        ...
                        ElezenImgui.AttachTooltip(isInTimeout ? "Timeout for refreshing active, please wait before refreshing again."
                ...
                if (_charaDataManager.LoadedMcdfHeader == null || _charaDataManager.LoadedMcdfHeader.IsCompleted)
                {
                    ...
                    if ((_charaDataManager.LoadedMcdfHeader?.IsCompleted ?? false))
                    {
                        ...
                        using (ImRaii.Disabled(!_hasValidGposeTarget))
                        {
                            ...
                            ElezenImgui.AttachTooltip($"Apply to {_gposeTarget}");
                            ImGui.SameLine();
    ...
    {
        _uiSharedService.BigText("MCDF Export");

        DrawHelpFoldout("This feature allows you to pack your character into a MCDF file and manually send it to other people. MCDF files can be imported into Snowcloak or Brio.");
    ...
    private void DrawMetaInfoData(string selectedGposeActor, bool hasValidGposeTarget, CharaDataMetaInfoExtendedDto data, bool canOpen = false)
    {
        ...
        UiSharedService.DrawGrouped(() =>
        {
            ...
            ElezenImgui.AttachTooltip($"Last Update: {data.UpdatedDate}");
    ...
    private void DrawPoseData(CharaDataMetaInfoExtendedDto? metaInfo, string actor, bool hasValidGposeTarget)
    {
        ...
        foreach (var item in metaInfo.PoseExtended)
        {
            ...
            if (!isInGpose)
            {
                ...
                ElezenImgui.AttachTooltip(tooltip + UiSharedService.TooltipSeparator + (item.HasWorldData ? GetWorldDataTooltipText(item) + UiSharedService.TooltipSeparator + "Click to show on Map" : string.Empty));
