...
internal class ReportPopupHandler : IPopupHandler
{
    ...
    public void DrawContent()
    {
        using (_uiSharedService.UidFont.Push())
            UiSharedService.TextWrapped(string.Format("Report {0} Profile", _reportedPair!.UserData.AliasOrUID));
        
        ImGui.InputTextMultiline("##reportReason", ref _reportReason, 500, new Vector2(500 - ImGui.GetStyle().ItemSpacing.X * 2, 200));
        UiSharedService.TextWrapped("Note: Sending a report will disable the offending profile globally.{0}The report will be sent to the team of your currently connected server.{0}Depending on the severity of the offense the users profile or account can be permanently disabled or banned."
        ...
        UiSharedService.ColorTextWrapped("This is not for reporting misbehavior but solely for the actual profile. Reports that are not solely for the profile will be ignored.", ImGuiColors.DalamudYellow);
